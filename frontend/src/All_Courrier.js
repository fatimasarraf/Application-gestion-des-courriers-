  ...
  useEffect(() => {
    const fetchAllCourriers = async () => {
      try {
        ...
        while (morePages) {
          const response = await fetch(`http://127.0.0.1:8000/api/Courrier/courriers/?user_id=${userId}&ordering=-sortie&page=${page}&page_size=100`);
          if (!response.ok) {
...
/>
  ...
  {courrier.is_read ? <FontAwesomeIcon icon={faEnvelopeOpen} /> : <FontAwesomeIcon icon={faEnvelope} />}
</span>
